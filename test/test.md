...
Device generates ES256 keypair (keeps private key).
Create Firestore doc /devices/{deviceId} with:
publicKey, ownerUid=null, saunaId=null, nonce=1, enabled=true.
Install & link (user setup in app)
App discovers device (BLE/SoftAP) and configures Wi-Fi on device.
App “claims” device → writes:
...
AUTH = Firebase Auth.
Flow
Device builds a short-lived deviceJWT:
Payload includes: deviceId, nonce, iat, exp (~60s).
Signed with device private key (ES256).
Device POST to BROKER with deviceJWT.
BROKER reads /devices/{deviceId} and verifies:
...
BROKER runs a Firestore transaction:
Assert payload.nonce == doc.nonce.
Update doc.nonce = nonce + 1, set lastSeenAt = now.
BROKER creates a custom token for ownerUid with claims:
{ role: "device", deviceId, saunaId }.
BROKER calls signInWithCustomToken to get:
id_token, optional refresh_token, expires_in.
BROKER → Device returns { idToken, refreshToken?, expiresIn }.
...
Safe-OFF outputs; load from NVS: compiled.today, currentSession, lastNonce, refreshToken?.
...
Read schedule if needed:
GET /saunas/{saunaId}/schedule only when local version differs.
Compute or load compiled intervals; cache compiled.today + currentSession.
Control loop:
...
Power loss
On restart:
Safe-OFF → load from NVS: compiled.today, currentSession.
If now < currentSession.end, resume ON until end (safety checks apply).
Wi-Fi → NTP → ensure auth (refresh or broker).
Flush queued telemetry (if persisted).
Check schedule.version and resync if changed.
...
Don’t store refresh token. When idToken expires (or on 401), device runs broker handshake again.
Pro: no long-lived secret on device. Con: hourly-ish broker call.
B) Refresh token (fewer broker calls)
Store refreshToken in NVS (encrypt if possible). Refresh ~10 min before expiry. Fall back to broker on failure.
Pro: less broker load. Con: treat refresh token like a password.
...
Nonce = “number used once.” Per-device counter in Firestore.
Device includes current nonce in signed deviceJWT.
Broker accepts only the exact expected value, then increments it in a Firestore transaction.
Prevents replay: a captured deviceJWT can’t be reused after the counter advances.
9) Operational notes
Broker service account (least privilege):
roles/iam.serviceAccountTokenCreator (on itself).
roles/datastore.user (to read /devices and run nonce transaction).
Keep broker warm: set minimum instances to reduce cold starts.
Kill switch: set /devices/{deviceId}.enabled=false to block new tokens immediately.
Clock hygiene: device uses NTP; broker accepts small skew with short exp (~60s).
10) Quick checklist
 Create /devices and /saunas collections with fields above.
 Deploy token broker Cloud Function (verify + nonce txn + custom token + exchange).
 Firestore Rules enforce owner or linked device access.
 ESP32 implements: JWT sign, broker call, token storage, schedule cache, telemetry queue.
 Reconnect logic: Wi-Fi backoff, token refresh/broker fallback, flush queue.
 Safety: Safe-OFF on boot, watchdog, over-temp cutoff.
Tip: If you ever want to collapse saunaId == deviceId for a strict 1:1 mapping, you can do that now and keep the data model ready for future decoupling by still storing both fields (equal at start) and using the server-side mapping in the broker claims.
